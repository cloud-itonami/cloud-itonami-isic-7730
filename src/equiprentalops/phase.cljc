(ns equiprentalops.phase
  "Phase 0->3 staged rollout for the ISIC-7730 machinery / equipment /
  tangible-goods rental operations-coordination actor.

    Phase 0  read-only            -- no writes, still governor-gated.
    Phase 1  assisted-logging     -- rental-record logging allowed,
                                      every write needs human approval.
    Phase 2  assisted-coordination-- adds post-return maintenance-
  ...
  `equiprentalops.governor`'s own `always-escalate-ops` enforces the
  same invariant independently -- two layers, not one, agree on this."
  (:require [equiprentalops.governor :as governor]))
...
(defn verdict->disposition
  "Map a EquipRentalGovernor verdict to a base disposition before
  the phase gate."
  [verdict]
