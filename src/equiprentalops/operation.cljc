(ns equiprentalops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (EquipRentalOpsAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the EquipRentalGovernor (:govern)
...
(defn build
  ...
  `equiprentalops.store/Store`).
  opts:
    :advisor      -- a `equiprentalops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; EquipRentalOpsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; EquipRentalGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
